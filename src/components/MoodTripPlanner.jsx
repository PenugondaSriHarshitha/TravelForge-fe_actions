...
const MOOD_EXAMPLES = {
  adventurous: { suggested: ["Rishikesh, India", "Queenstown, NZ", "Bali, Indonesia"], tagline: "High-energy days with hikes, water sports and offbeat routes." },
  romantic:    { suggested: ["Santorini, Greece", "Paris, France", "Udaipur, India"], tagline: "Candlelit dinners, scenic sunsets and quiet boutique stays." },
  relaxing:    { suggested: ["Goa, India", "Maui, USA", "Kerala, India"], tagline: "Slow mornings, spa time and seaside lounging." },
  foodie:      { suggested: ["Lyon, France", "Tokyo, Japan", "Bangkok, Thailand"], tagline: "Local markets, chef tables and street food crawls." },
  cultural:    { suggested: ["Kyoto, Japan", "Istanbul, Turkey", "Jaipur, India"], tagline: "Temples, neighbourhood walks and immersive local experiences." },
  nature:      { suggested: ["Patagonia, Chile", "Sikkim, India", "Banff, Canada"], tagline: "Wild landscapes, quiet lakes and starry nights." },
};
...
function synthesizeItinerary({ mood, destination, start, end, budget }) {
  ...
  for (let i = 1; i <= daysCount; i++) {
    ...
    for (let j = 0; j < count; j++) {
      let candidate;
      do { candidate = pick(pool); } while (used.has(candidate.title) && used.size < pool.length);
  ...
  base.summary = `${base.mood[0].toUpperCase() + base.mood.slice(1)} trip to ${base.destination} • ${base.days.length} days • est ${base.hotel.price_est}`;
  return base;
}
 ...
 // Start closed by default — only open when explicitly triggered
  ...
  useEffect(() => {
    ...
    style.innerHTML = `
      ...
      @media (max-width:760px){ .mood-overlay{left:12px;right:12px;width:auto;top:60px;max-height:86vh} .mood-grid{grid-template-columns:1fr} .mood-day{flex-direction:column} .day-image{width:100%;height:160px} }
    `;
    document.head.appendChild(style);
    return () => {
      const el = document.getElementById("mood-trip-styles");
      if (el) el.remove();
    };
  }, []);

  // lock body scroll while planner open
  useEffect(() => {
    if (open) {
  ...
  // Allow external "Plan by mood" triggers to open the planner:
  useEffect(() => {
    const openHandler = (e) => {
    ...
    try {
      ...
      plan.days = plan.days.map((d, i) => ({
        ...
        hidden_gems: [{
          ...
          google_maps: `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${plan.destination} ${d.title}`)}`
        }]
      }));
      setItinerary(plan);
    } catch (err) {
  ...
  const handleDownload = () => {
    ...
    const a = document.createElement("a"); a.href = url; a.download = `${slugify(itinerary.destination || "mood-trip")}.json`; a.click(); URL.revokeObjectURL(url);
  };

  const handleShare = () => {
    if (!itinerary) return alert("Generate an itinerary first.");
    if (navigator.share) navigator.share({ title: `My ${itinerary.mood} trip to ${itinerary.destination}`, text: itinerary.summary, url: window.location.href }).catch(()=>{});
    else navigator.clipboard.writeText(`${itinerary.summary}\n\n${JSON.stringify(itinerary, null, 2)}`).then(()=> alert("Copied itinerary summary to clipboard")).catch(()=> alert("Share not supported here — download instead"));
  };

  const handleExplore = () => { const slug = slugify(destination || (itinerary && itinerary.destination) || "destination"); navigate(`/explore/${slug}?source=mood`); setOpen(false); };

  // If closed, show small launcher
  if (!open) {
    return (
      <div style={{ position: "fixed", right: 20, top: 96, zIndex: 99999 }}>
        <button onClick={() => setOpen(true)} style={{ padding: "12px 14px", borderRadius: 12, border: "none", background: "linear-gradient(90deg,#3ee7a4,#ffb86b)", color: "#042f28", fontWeight: 900, boxShadow: "0 10px 30px rgba(62,231,164,0.12)", cursor: "pointer" }} aria-label="Open Mood Trip Planner">Open Mood Trip Planner</button>
      </div>
    );
  }
        ...
        <div className="mood-head">
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <div>
              ...
              <div className="mood-sub">{MOOD_EXAMPLES[mood]?.tagline || "Personalized mood itineraries"}</div>
            </div>
          </div>

          <div className="mood-controls">
            <button className="action-small" onClick={() => setOpen(false)}>Hide</button>
            <button className="action-small" onClick={handleSurprise}>Surprise me</button>
            {/* Back navigates to home (so it always lands on your home page) */}
            <button className="action-small" onClick={goHome}>Back</button>
          </div>
        </div>
        ...
        <div className="mood-body" aria-live="polite">
          <div className="mood-row" role="tablist" aria-label="Mood selector">
            {Object.keys(MOOD_EXAMPLES).map((m) => (
              <button
                key={m}
                className={`chip ${m === mood ? "active" : ""}`}
                onClick={() => { setMood(m); if (!open) setOpen(true); }}
                aria-pressed={m === mood}
              >
                {m}
              </button>
          ...
          <div className="mood-grid">
            <input className="mood-input" placeholder="Destination (leave blank to auto-suggest)" value={destination} onChange={(e) => setDestination(e.target.value)} aria-label="Destination" />
            <input className="mood-input" type="number" min={50} placeholder="Budget per person (USD)" value={budget} onChange={(e) => setBudget(Number(e.target.value || 0))} aria-label="Budget" />
            <input className="mood-input" type="date" value={start} onChange={(e) => setStart(e.target.value)} aria-label="Start date" />
            <input className="mood-input" type="date" value={end} onChange={(e) => setEnd(e.target.value)} aria-label="End date" />
          </div>

          <div style={{ display: "flex", gap: 10, marginBottom: 6 }}>
            <button className="btn-generate" onClick={() => generate(false)} disabled={loading} aria-busy={loading}>{loading ? "Thinking…" : "Generate itinerary"}</button>
            <button className="btn-ghost" onClick={() => { setDestination(""); setStart(""); setEnd(""); setItinerary(null); setError(null); }}>Reset</button>
          </div>
          ...
          {loading && (
            <div style={{ marginTop: 14 }}>
              <div style={{ height: 12, width: "60%", background: "#eee", borderRadius: 6, marginBottom: 10 }} />
              <div style={{ height: 10, width: "30%", background: "#eee", borderRadius: 6, marginBottom: 18 }} />
              {[0,1,2].map(i => (
                <div key={i} style={{ display: "flex", gap: 12, marginBottom: 12, alignItems: "flex-start" }}>
                  <div style={{ width: 8, height: 8, borderRadius: 8, background: "#eee", marginTop: 6 }} />
                  <div style={{ flex: 1 }}>
                    <div style={{ height: 10, width: `${60 - i*8}%`, background: "#eee", borderRadius: 6, marginBottom: 8 }} />
                    <div style={{ height: 10, width: `${40 - i*6}%`, background: "#eee", borderRadius: 6 }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          {itinerary && !loading && (
            <div className="mood-result" role="region" aria-label="Generated itinerary">
              <div className="mood-summary">
                ...
                <div style={{ color: "var(--muted)" }}>{itinerary.dates.start || ""}{itinerary.dates.end ? ` — ${itinerary.dates.end}` : ""}</div>
              </div>

              {itinerary.days.map((d, idx) => (
                <div key={idx} className="mood-day" aria-labelledby={`day-${idx}`}>
                  {d.image ? <img src={d.image} alt={`Day ${d.day}`} className="day-image" onError={(e)=> e.currentTarget.style.display='none'} /> : <div style={{ width: 120, height: 80, borderRadius: 10, background: "#fff" }} />}
                  <div style={{ flex: 1 }}>
                    ...
                    {d.hidden_gems && d.hidden_gems.map((g, gi) => (
                      <div key={gi} style={{ marginTop: 8, color: "#2d635b", fontWeight: 700 }}>
                        ✨ Hidden gem: <a href={g.google_maps} target="_blank" rel="noreferrer">{g.name}</a> — <span style={{ fontWeight: 500, color: "var(--muted)" }}>{g.reason}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div style={{ marginTop: 12, fontWeight: 700 }}>
                🏨 Suggested hotel: {itinerary.hotel.name} • <span style={{ fontWeight: 600 }}>{itinerary.hotel.price_est}</span>
              </div>

              <div style={{ marginTop: 12, display: "flex", gap: 8 }}>
                <button className="btn-generate" onClick={handleShare}>Share</button>
                <button className="btn-ghost" onClick={handleDownload}>Download JSON</button>
                <button className="btn-ghost" onClick={handleExplore}>Explore</button>
              </div>
            </div>
          )}
        </div>
